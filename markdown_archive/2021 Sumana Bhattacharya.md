...
## INTRODUCTION
Extraterrestrial soil research is at the core of planetary missions to understand the geomechanical properties including the soil bearing resistance against surface loading. This effort is vital to enable successful future surface activities such as the human habitat construction (Lim et al. 2017). 
...
Considering the cohesion of lunar soil is relatively low, the effect of the first term ($cN_c$) is negligible. Since $\gamma = \rho g$ (where $\rho$ is soil density and $g$ is gravitational acceleration), the formula for partial gravity ($g/n$) can be expressed as:

$$q_d = \rho(g/n)(D_f N_q + 0.5 B N_\gamma)$$ (2)

This study hypothesizes that soil bearing resistance can be reproduced in Earth gravity ($g$) by proportionally scaling the particle density to $\rho/n$:

$$q_d = (\rho/n)g(D_f N_q + 0.5 B N_\gamma)$$ (3)

## DISCRETE ELEMENT MODELING
An open source DEM code, **LIGGGHTS**, was used. Toyoura sand with 60% relative density (void ratio ~ 0.76) was selected. To keep computational costs manageable, particles were scaled to spheres of 8 mm diameter (approx. 6770 particles used).
...
The DEM simulations were performed for reduced gravity conditions ($1/2g, 1/6g$) to validate soil behavior. Then, density was scaled to $1/2\rho$ and $1/6\rho$ in Earth gravity ($1g$).
- **Figure 3** shows calibration in $1g$ matching experimental data.
- **Figure 6** compares simulated bearing resistance of $1/2\rho$ in $1g$ to $1\rho$ in $1/2g$. Overall good agreement was found.
- **Figure 7** shows bearing resistance of $1/6\rho$ in $1g$ is comparable to $1\rho$ in $1/6g$.

## CONCLUDING REMARKS
This study leverages Terzaghi’s bearing capacity theory to demonstrate that the gravity effect on soil bearing resistance can be reproduced by adopting a synthetic soil with proportionally scaled particle density. This approach provides an inexpensive alternative to parabolic flights. With advances in 3D printing, synthetic particles with controlled density can be produced to simulate extraterrestrial soil behavior in standard laboratory settings.